...
namespace STI.Biometria.Dominio.Infra.Integracoes.Testes
{
    public sealed class DigitaisDataAccess
    {
        ...
        public Digital CarregarDigitalEncontrada()
        {
            using (var conexao = new SqlConnection(_stringConexao))
            {
                var sql = @"SELECT id, matricula, templateISO FROM Digital_Escopo_Testes (NOLOCK) WHERE ModoTeste = 'VALID'";
        ...
        public Digital CarregarDigitalNaoEncontrada()
        {
            using (var conexao = new SqlConnection(_stringConexao))
            {
                var sql = @"SELECT id, matricula, templateISO FROM Digital_Escopo_Testes (NOLOCK) WHERE ModoTeste = 'INVALID'";
                return conexao.Query<Digital>(sql, new { }).FirstOrDefault();
            }
        }        
    }
